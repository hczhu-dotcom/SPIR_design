...
def candidate_filtering(args):
    ...
    for each_pdb in os.listdir(f'{input_folder}/Accepted'):
        ...
        antigen_sequence, binder_sequence = nanobody_analysis.get_sequence_from_antigen_binder_pdb(pdb_path = pdb_path)
        #获取信息的同时 change chain name, target_name = 'T',binder_name = 'B'

        #计算binder的N末端 C末端之间的距离
        nt_ct_distance_dict = nanobody_analysis.calculate_nt_ct_distance(pdb_file = pdb_path, chain_id = 'B')
    ...
    #然后extra align
    #nanobody_analysis.extra_alignment(input_folder = f'{input_folder}/Accepted',only_align = True)
